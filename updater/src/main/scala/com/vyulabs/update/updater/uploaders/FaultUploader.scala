...
class FaultUploader(archiveDir: File, clientDirectory: ClientDistributionDirectoryClient)
  ...
  private def uploadFault(fault: FaultReport): Boolean = {
    try {
      val serviceDir = new File(archiveDir, UpdateUtils.serializeISO8601Date(new Date))
      if (!serviceDir.mkdir()) {
        log.error(s"Can't create directory ${serviceDir}")
        return false
      }
      val archivedFileName = s"${fault.state.serviceInstanceName.serviceName}_${fault.state.version.getOrElse(BuildVersion.empty)}_${fault.instanceId}_${UpdateUtils.serializeISO8601Date(new Date())}_fault.zip"
      ...
      try {
        if (!UpdateUtils.writeConfigFile(stateInfoFile, fault.state.toConfig())) {
          log.error(s"Can't write file with state")
          return false
        }
        val logs = fault.logTail.foldLeft(new String) { (sum, line) => sum + '\n' + line }
        if (!UpdateUtils.writeFileFromBytes(logTailFile, logs.getBytes("utf8"))) {
          log.error(s"Can't write file with tail of logs")
          return false
        }
        val filesToZip = fault.reportFilesTmpDir.toSeq :+ stateInfoFile :+ logTailFile
        if (!UpdateUtils.zip(archiveFile, filesToZip)) {
          log.error(s"Can't zip ${filesToZip} to ${archiveFile}")
          return false
        }
      } finally {
        UpdateUtils.deleteFileRecursively(tmpDirectory)
      }
      fault.reportFilesTmpDir.foreach(UpdateUtils.deleteFileRecursively(_))
      ...
      UpdateUtils.maybeFreeSpace(serviceDir, maxServiceDirectoryCapacity, Set(archiveFile))
      true
    } catch {
      case ex: Exception =>
