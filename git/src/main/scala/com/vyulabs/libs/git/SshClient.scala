...
/**
  ...
  */
class SshClient(host: String, account: String, port: Int = 22)(implicit log: org.slf4j.Logger) {
  ...
  private val session = sch.getSession(account, host, port)
...
object SshClient {
  def apply(host: String, account: String = "", port: Int = 22)(implicit log: org.slf4j.Logger): SshClient = {
    new SshClient(host, account, port)
  }
}