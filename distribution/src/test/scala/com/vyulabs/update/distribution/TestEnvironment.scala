...
abstract class TestEnvironment(createIndices: Boolean = false) extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  result(for {
    ...
    _ <- collections.Developer_Services.insert(BuildServiceConfig("", Some(distributionName), Seq.empty, Seq.empty, Seq.empty, Seq.empty))
    _ <- collections.Client_Services.insert(BuildServiceConfig("", Some(distributionName), Seq.empty, Seq.empty, Seq.empty, Seq.empty))
  } yield {})
