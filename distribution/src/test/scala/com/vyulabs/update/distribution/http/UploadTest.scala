...
class UploadTest extends TestEnvironment with ScalatestRouteTest {
  ...
  it should "upload developer version image" in {
    Post("/load/developer-version-image/service1/test-1.1.1", makeVersionMultipart()) ~> addCredentials(builderClientCredentials) ~> route ~> check {
  ...
  it should "upload client version image" in {
    Post("/load/client-version-image/service1/test-1.1.1_1", makeVersionMultipart()) ~> addCredentials(builderClientCredentials) ~> route ~> check {
      status shouldEqual StatusCodes.OK
    }
