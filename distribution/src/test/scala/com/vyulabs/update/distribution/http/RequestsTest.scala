...
class RequestsTest extends TestEnvironment with ScalatestRouteTest {
  ...
  it should "process graphql post request" in {
    Post("/graphql", """{ "query": "query { userAccountsInfo(account:\"admin\") { account, role } }" }""".parseJson) ~> addCredentials(adminHttpCredentials) ~> route ~> check {
      status shouldEqual StatusCodes.OK
      responseAs[String] shouldEqual """{"data":{"userAccountsInfo":[{"account":"admin","role":"Administrator"}]}}"""
    }
  }

  it should "process graphql get request" in {
    Get(s"/graphql?query=" + URLEncoder.encode("""{ userAccountsInfo(account:"admin") { account, role } }""", "utf8")) ~> addCredentials(adminHttpCredentials) ~> route ~> check {
      status shouldEqual StatusCodes.OK
      responseAs[String] shouldEqual """{"data":{"userAccountsInfo":[{"account":"admin","role":"Administrator"}]}}"""
    }
  }
  ...
  it should "process change password request" in {
    Get(s"/graphql?query=" + URLEncoder.encode("""mutation { changeUserAccount ( oldPassword: "admin", password: "password1" ) }""", "utf8")) ~> addCredentials(adminHttpCredentials) ~> route ~> check {
      status shouldEqual StatusCodes.OK
    }

    val newAdminClientCredentials = BasicHttpCredentials("admin", "password1")

    Post("/graphql", """{ "query": "query { userAccountsInfo(account:\"admin\") { account, role } }" }""".parseJson) ~> addCredentials(newAdminClientCredentials) ~> route ~> check {
      status shouldEqual StatusCodes.OK
      responseAs[String] shouldEqual """{"data":{"userAccountsInfo":[{"account":"admin","role":"Administrator"}]}}"""
    }
  }
