...
trait DeveloperVersionUtils extends DistributionConsumersUtils with DistributionConsumerProfilesUtils
  ...
  def buildDeveloperVersion(serviceName: ServiceName, developerVersion: Version, author: UserName,
  ...
  def removeDeveloperVersion(serviceName: ServiceName, version: DistributionVersion)(implicit log: Logger): Future[Boolean] = {
  ...
  def getDeveloperVersionsInfo(serviceName: ServiceName, distributionName: Option[DistributionName] = None,
                               version: Option[Version] = None)(implicit log: Logger): Future[Seq[DeveloperVersionInfo]] = {
  ...
  def getDeveloperDesiredVersion(serviceName: ServiceName)(implicit log: Logger): Future[Option[DistributionVersion]] = {
  ...
  private def getBusyVersions(distributionName: DistributionName, serviceName: ServiceName)(implicit log: Logger): Future[Set[Version]] = {
    for {
      ...
      (desiredVersion.toSet ++ installedVersions ++ testedVersions).filter(_.distributionName == distributionName).map(_.build)
    }
  }
}