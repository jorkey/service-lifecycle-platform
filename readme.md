...
- _network_
  ...
  - `ssl` (необязательно, для сервиса https)
...
В репозитории разработки нужно создать файл _update.json_, описывающий правила сборки, 
установки и запуска сервисов.
    ...
    - _install_
        ...
        - `runService` - правила запуска сервиса
          ...
          - `restartConditions` - условия для принудительного завершения сервиса
            ...
            - `makeCore:boolean` : для Unix-подобных систем, завершение сервиса сигналом SIGQUIT, для образования core dump  
            - `checkTimeoutMs:number` :  интервал проверки условий 

В стоковых значениях настроек могут присутствовать макро, которые заменяются при обработке
на реальные значения:
- _%%version%%_ - версия сборки
- _%%profile%%_ - профиль установки сервиса (см ???) 

## Определение конфигурации клиента

Секция _update/install_ из файла _update.json_ записывается в файл _install.json_ и включается в образ версии.
Клиент может дополнить или изменить эту секцию. Для этого в каталоге services/<service> административного репозитория создаётся файл install.json.
