...
### Откат до предыдущей версии списка

При нажатии на кнопку _Rollback_ появляется панель управления версиями списка желаемых версий. Версии помечаются временем создания.

Текущая версия находится посередине, слева более старая, справа более новая. Чтобы перейти к более старой версии, нажмите '<-',
для перехода к более новой версии нажмите '->'. При изменении версии меняется содержимое таблицы внизу, изменившиеся записи помечаются следующим образом:
- жирным шрифтом - у сервиса изменилась желаемая версия
- красным цветом - в этой версии нет записи с этим сервисом
- зелёным цветом - сервис добавился

Для назначения списка желаемых версий текущим, нажмите _Save_. 

### Назначение версий вручную

Также можно произвольно назначать версии сервисам. Для этого нужно кликнуть по версии сервиса, после чего появится список доступных версий в истории.
