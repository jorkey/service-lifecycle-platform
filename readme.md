...
Для создания версий запускается _builder_ локально или на другом сервере дистрибуции.
...
Схема GraphQL предоставляет запрос для поиска записей журналов по разным критериям.
Можно также выполнить GraphQL subscription на журнал записей, и тогда новые записи в журнал будут
...
- _payload_
  ...
  - _info_ - последнее состояние сервиса
    ...
    - _logTail:LogLine[]_ - 500 последних записей в журнал
...
_Provider_ предоставляет для _consumer_ заданные в его _profile_ сервисы.
...
_Consumer_ отправляет _provider_-у информацию об установленных сервисах _provider_-а:
- Номера установленных версий
- Состояние инстанций
- Отчёты о сбоях

_Provider_ также может выполнять _builder_ для _consumer_.
Это необходимо, если на сервере _consumer_ нет достаточных ресурсов для сборки версии, или _provider_
обладает необходимым набором системных библиотек.

Возможно автоматическое обновление сервисов _consumer_ при появлении новых версий разработчика на _provider_.
_Consumer_ периодически проверяет наличие обновлений, и если обновления обнаружены, запускает задачу обновления.
...
Сервис пишет журнал работы на стандартный вывод. Формат строки журнала должен быть следующим:
...
- Останавливается старая версия
  - Процессу сервиса и его потомкам отправляется сигнал 
...
Определяются именуемые группы сервисов, или profiles, как подмножества списка сервисов для разработки.
...
- _Auto Update_ - При изменении желательных версий на _distribution provider_ они автоматически
