    ...
    - _install_
      ...
      - _[runService]_ - правила запуска сервиса
        ...
        - _[restartConditions]_ - условия для принудительного завершения сервиса
          ...
          - _[checkTimeoutMs:number]_ : интервал проверки условий
...
- _macroValues_ - значения макро в настройках в Json формате:
  - []
    - _name_ - имя
    - _value_ - значение
...
- _settingsRepositories_ - репозитории с настройками в Json формате:
  - []
    ...
    - _git_
      ...
      - _branch_ - branch Git репозитория
      - _[cloneSubmodules]_ - клонировать вместе с submodules
- _macroValues_ - значения макро в настройках в Json формате как в _buildDeveloperVersion_
...
- Дополнительные настройки из подкаталогов `config` сливаются с настройками версии разработчика 
  (см [Кастомизация настроек для клиента](#settings_customization))
- Файлы из подкаталогов `private` копируются в каталог сборки.
  Там хранятся обычно ключи, сертификаты, и другая приватная информация.
