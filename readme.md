...
Сервер дистрибуции принимает запросы GraphQL от пользователей, от других сервисов vUpdate 
и других серверов дистрибуции.
Запросы GraphQL можно поделить на группы:
- Создание/редактирование/поиск/удаление
  - учётных записей
  - репозиториев исходных кодов
  - профилей сервисов
- Создание/поиск
  - версий разработчика
  - желательных версий разработчика
  - версий клиента
  - желательных версий клиента



#### Задачи

Задача на сервере дистрибуции имеет уникальный ID, которым помечаются все записи в журнале, 
относящиеся к выполнению этой задачи. Последняя запись, относящаяся к этой задаче, содержит
_terminationStatus_.
...
_level_ - уровень отладки: TRACE, DEBUG, INFO, WARN или ERROR
...
#### Отправка состояния сервиса

Раз в 10 сек _updater_ отправляет состояние сервисов на сервер дистрибуции. 

Состояние сервиса состоит из:
...
В отчёте содержится:
- Последнее состояние сервиса
- 500 последних записей в журнал
- Файлы с информацией о сбое
