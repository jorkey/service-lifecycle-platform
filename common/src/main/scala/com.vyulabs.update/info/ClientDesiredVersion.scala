...
case class ClientDesiredVersion(serviceName: ServiceName, buildVersion: ClientDistributionVersion)
...
object ClientDesiredVersions extends DefaultJsonProtocol {
  ...
  def toMap(versions: Seq[ClientDesiredVersion]): Map[ServiceName, ClientDistributionVersion] = {
    versions.foldLeft(Map.empty[ServiceName, ClientDistributionVersion])((map, version) => map + (version.serviceName -> version.buildVersion))
  }
