...
case class DeveloperDesiredVersion(serviceName: ServiceName, version: DistributionVersion)
...
object DeveloperDesiredVersions extends DefaultJsonProtocol {
  ...
  def toMap(versions: Seq[DeveloperDesiredVersion]): Map[ServiceName, DistributionVersion] = {
    versions.foldLeft(Map.empty[ServiceName, DistributionVersion])((map, version) => map + (version.serviceName -> version.version))
  }

  def fromMap(versions: Map[ServiceName, DistributionVersion]): Seq[DeveloperDesiredVersion] = {
...
case class DeveloperDesiredVersionDelta(serviceName: ServiceName, version: Option[DistributionVersion])
